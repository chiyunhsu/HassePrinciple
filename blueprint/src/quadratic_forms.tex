...
If $n=1$, then $\epsilon(f) = 1$. If $n=2$, then $\epsilon(f) = 1$ if and only if the form $Z^2 - a_1 X^2 - a_2 Y^2$ represents $0$. By Corollary \ref{cor:represents_iff}, this is equivalent to $a_1 X^2 + a_2 Y^2$ representing $1$, which means that there exists $v \in V$ with $Q(v) = 1$; this is independent on $b$. 
...
\begin{theorem}[Hasse--Minkowski]\label{lem:HM_of_degenerate}
	\lean{QuadraticForm.HasseMinkowski}
	\uses{lem:ev_loc_isotr_of_isotr, lem:isotr_of_rk_zero, lem:isotr_of_rk_one, lem:isotr_of_rk_two, lem:isotr_of_rk_three, lem:isotr_of_rk_four, lem:isotr_of_five_le_rk}
	\leanok
...
nondegenerate quadratic form. Then $f$ is equivalent to a form $X_1^2 +  a_2X_2^2 + \cdots + a_nX_n^2$, for some $a_i \in K^\times$.
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:equiv_sum_squares_sqfree}
	\lean{QuadraticForm.equivalent_weightedSumSquares_squarefree_units_of_nondegenerate}
	\uses{lem:equiv_sum_squares}
	\leanok
	Let $K$ be a field of characteristic not equal to $2$, let $M$ be a $K$-vector space of positive dimension $n$ and let $f : M \to K$ be a
	nondegenerate quadratic form. Then $f$ is equivalent to a form $X_1^2 +  a_2X_2^2 + \cdots + a_nX_n^2$, for some squarefree $a_i \in K^\times$.
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:isotr_of_rk_three}
	...
	\uses{def:everywhere_locally_isotropic, lem:equiv_sum_squares, prop:common_root}
	\leanok
...
\uses{def:everywhere_locally_isotropic, lem:equiv_sum_squares, cor:prod_isotropic_iff, thm:product_formula,
cor:represents_iff, lem:isotr_of_rk_three}
\leanok
...
This shows that the form $aX_1^2 + bX_2^2 - xZ^2$ represents zero in each $\Q_v$, so by Lemma \ref{lem:isotr_of_rk_three}, it also represents $0$ in $\Q$. By Corollary \ref{cor:represents_iff}, $x$ is represented by $aX_1^2 + bX_2^2$. The same argument shows that $x$ is represented by $cX_1^2 + dX_2^2$. By applying Corollary \ref{cor:prod_isotropic_iff} again, we conclude that $f$ represents $0$.
\end{proof}

\begin{lemma}\label{lem:isotr_of_five_le_rk}
\lean{QuadraticForm.EverywhereLocallyIsotropic.isotropic_of_five_le_rank}
\uses{def:everywhere_locally_isotropic, lem:equiv_sum_squares, thm:padic_squares_open, thm:approximation, def:HM_inv, lem:isotr_of_rk_four}
\leanok
...
Consider the form $f_1 := cZ^2 - g$. For each $v \in S$, $g$ represents $c_v$ in $\Q_v$, and hence also $c$ because $c/c_v \in {\Q_v^\times}^2$, hence $f_1$ represents $0$ in $\Q_v$ by Corollary \ref{cor:represents_iff}. For $v \notin S$, the coefficients $-a_3, \dots, -a_n$ of $g$ are $v$-adic units, so the discriminant $d_v(g)$ is a $v$-adic unit and $\epsilon_v(g)=1$.

In all cases, $f_q$ represents $0$ in all $\Q_v$, and since the rank of $f$ is $n-1$, the inductive hypothesis shows that $f_1$ represents $0$ in $\Q$, or equivalently by Corollary \ref{cor:represents_iff}, $g$ represents $c$ in $\Q$. But $h$ also represents $c$, so  by Corollary \ref{cor:prod_isotropic_iff} $f$ represents $0$.
