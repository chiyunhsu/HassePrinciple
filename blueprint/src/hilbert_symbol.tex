...
\begin{definition}
	...
	\lean{HilbertSymbol}
	\leanok
	Let $k$ be a field and $a, b \in k^\times$. The \emph{Hilbert symbol} $(a, b)_k$ is defined as
	follows: if there exist $x, y, z \in k$, not all zero, such that $z^2 = ax^2 + by^2$,
	then $(a, b)_k = 1$; otherwise, $(a, b)_k = -1$.
\end{definition}
...
\begin{proof}
	...
	$z^2 = bx^2 + ay^2$.
		...
		N.B. Alternatively: the same argument as in the first case of the proof of Lemma
		\ref{lem:eq_one_iff} applies.
...
TODO: should we move all these to a Padic section?
\begin{definition}
	\label{def:epsilon}
...
\begin{proof}
	\uses{def:Hilbert_symbol, def:at_p, def:at_infty, thm:chineseRemainder, thm:approximation}
