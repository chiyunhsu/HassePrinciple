...
lemma isSquare_of_even_factorization {n : ℕ}
  ...
  letI : Fact (Nat.Prime p) := ⟨Nat.prime_of_mem_primeFactors hp⟩
  rw [powMonoidHom_apply, ← pow_mul, Nat.div_two_mul_two_of_even (h p)]
...
lemma isSquare_of_nonneg_of_even_factorization {n : ℤ} (h0 : 0 ≤ n)
  ...
  exact ⟨r, (by rw [← Int.natAbs_of_nonneg h0, hr, Nat.cast_mul])⟩
