...
private lemma all_but_one_places_suffice
  ...
  by_cases hpq : p = q
  · --We call HS_supp and ei_supp the supports of the Hilbert symbol and of ep i, respectively.
    let HS_supp := (almost_all_one x (a i)).toFinset
    let ei_supp := (h1 i).toFinset
    --The two supports differ by at most one element.
    have symm_diff_card_eq_one : (symmDiff HS_supp ei_supp).card ≤ 1 := by
      refine Finset.card_le_one_iff_subset_singleton.mpr ⟨q, fun p ↦ ?_⟩
      contrapose
      simp only [symmDiff, Finset.mem_singleton]
      intro hp
      simp [HS_supp, ei_supp, ← ep_eq_neg_one_iff_not_one hep1 i p, (hx i).1 p hp]
    --If the two supports are identical, in particular hilbertSym and ei i agree everywhere.
    interval_cases h0 : (symmDiff HS_supp ei_supp).card
    · simp only [Finset.card_eq_zero, Finset.symmDiff_eq_empty] at h0
      by_cases hqsupp : q ∈ HS_supp
      · have : q ∈ ei_supp := by simp [← h0, hqsupp]
        simp [← eq_neg_one_iff_not_one, HS_supp, ← ep_eq_neg_one_iff_not_one hep1 i q,
          ei_supp] at hqsupp this
        lia
      · have : q ∉ ei_supp := by simp [← h0, hqsupp]
        simp [HS_supp, ei_supp] at hqsupp this
        lia
    --If not, we will show a contradiction.
    · --We cancel our the real place from the product formulas for hilbertSym and ei. The products
      --still agree.
      have prodHS_fin_eq_prode_fin :
        ∏ᶠ (p : Nat.Primes), hilbertSym (x : ℚ_[p]) (a i) =
          ∏ᶠ (p : Nat.Primes), ep i p := by
        rw [← mul_left_inj' (by grind : ereal i ≠ 0)]
        nth_rw 1 [← (hx i).2, hilbertSym.prod_eq_one x (a i), h2 i]
      --Each product is equal to (-1) raised to the cardinality of the support.
      have LHS :
          ∏ᶠ (p : Nat.Primes), hilbertSym (x : ℚ_[p]) (a i) = (-1) ^ HS_supp.card := by
        calc ∏ᶠ (p : Nat.Primes), hilbertSym (x : ℚ_[p]) (a i)
          _ = ∏ᶠ (p ∈ HS_supp), hilbertSym (x : ℚ_[p]) (a i) := by simp [HS_supp]
          _ = ∏ᶠ (p ∈ HS_supp), -1 := by simp [← eq_neg_one_iff_not_one, HS_supp]; grind
          _ = (-1) ^ HS_supp.card := by simp [finprod_mem_finset_eq_prod]
      have RHS : ∏ᶠ (p : Nat.Primes), ep i p = (-1) ^ ei_supp.card  := by
        calc ∏ᶠ (p : Nat.Primes), ep i p
          _ = ∏ᶠ (p ∈ ei_supp), ep i p := by simp [ei_supp]
          _ = ∏ᶠ (p ∈ ei_supp), -1 := by simp [← ep_eq_neg_one_iff_not_one hep1, ei_supp]; grind
          _ = (-1) ^ ei_supp.card := by simp [finprod_mem_finset_eq_prod]
      --Simplifying, we obtain that HS_supp.card and ei_supp.card have the same parity.
      rw [LHS, RHS] at prodHS_fin_eq_prode_fin
      have : HS_supp.card % 2 = ei_supp.card % 2 := by
        simp [neg_one_pow_eq_ite] at prodHS_fin_eq_prode_fin
        split_ifs at prodHS_fin_eq_prode_fin with parityHS parityei <;> grind
      --Since H_supp and ei_supp differ by one element (q), this is a contraidciton.
      simp only [symmDiff, Finset.sup_eq_union', Finset.card_union_of_disjoint
        (by simp [disjoint_sdiff_sdiff] : Disjoint (HS_supp \ ei_supp) (ei_supp \ HS_supp)),
        Nat.add_eq_one_iff, Finset.card_eq_zero, Finset.sdiff_eq_empty_iff_subset] at h0
      rcases h0 with ⟨ss,diff⟩ | ⟨diff,ss⟩
      all_goals {
        rw [Finset.card_sdiff_of_subset ss] at diff
        grind
        }
  · exact (hx i).1 p hpq
  ...
  · have pprime_fact : Fact (Nat.Prime p) := by
    ...
    · --Strategy: x is a square mod 8, so it's a square in ℚ_[2], hence (x,_)₂=1.
      ...
      have e2 : ep i ⟨p,pprime⟩ = 1 := by
        ...
        have : ¬ ∃ j, ep j ⟨2, Nat.prime_two⟩ = -1 := t_in_T_iff.mp two_notin_T
      ...
      · --If p ∉ S, then all the a_i are p-adic units.
        ...
        · have val_x : padicValRat p x.val = 1 := by
            ...
            simp only [ne_comm, ne_eq,
              ← Nat.prime_dvd_prime_iff_eq pprime (Subtype.forall.mp primes_T t htnep.1)] at htnep
            exact htnep.2
        ...
        · --In this case, the exponent is 0, so (x,a_i)ₚ = 1. Since p ∉ T, ep i p = 1 as well.
          have val_x : padicValRat p x.val = 0 := by
            ...
            apply Prime.not_dvd_finsetProd (Nat.prime_iff.mp pprime)
            intro t htinT hdvd
            rw [Nat.prime_dvd_prime_iff_eq pprime (primes_T t)] at hdvd
            rw [hdvd] at hpT
            simp at hpT
          qify
  ...
  · simp only [ne_eq, Rat.cast_eq_zero, Units.ne_zero, not_false_eq_true, real_eq, Rat.cast_pos,
    ...
    have : 0 < x.val := by
      simp only [Finset.univ_eq_attach, Nat.cast_mul, Nat.cast_prod, IsUnit.val_unit', Nat.cast_pos,
        Nat.Prime.pos q_prime, mul_pos_iff_of_pos_right, x, xQ, A]
      apply Finset.prod_pos
      intro t ht
      exact_mod_cast Nat.Prime.pos (primes_T t)
    simp [this]
